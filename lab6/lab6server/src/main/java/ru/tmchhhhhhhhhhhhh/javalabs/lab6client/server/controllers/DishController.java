package ru.tmchhhhhhhhhhhhh.javalabs.lab6client.server.controllers;
...
public class DishController {
    ...
    private final Map<String, Dish> menu = new ConcurrentHashMap<>();
    private final Map<String, Integer> orderCount = new ConcurrentHashMap<>();
    private final Map<String, Integer> caloriesMap = new ConcurrentHashMap<>();
    private final List<String> menuChangeLog = new CopyOnWriteArrayList<>();
    ...
    private void initTestData() {
        ...
        System.out.println("[INIT] Тестовые данные загружены. Меню содержит " + menu.size() + " блюд");
    }

    private synchronized void createDish(Dish dish) {
    ...
    public synchronized Response createDishFromRequest(Request request) {
        try {
    ...
    public synchronized Response updateDish(Request request) {
        try {
    ...
    public synchronized Response deleteDish(Request request) {
        try {
            JsonObject json = gson.fromJson(request.getData(), JsonObject.class);
            String id = json.get("id").getAsString();
    ...
    public synchronized Response orderDish(Request request) {
        try {
            JsonObject json = gson.fromJson(request.getData(), JsonObject.class);
            String id = json.get("id").getAsString();
    ...
    private void addToLog(String message) {
        ...
        menuChangeLog.add(0, "[" + timestamp + "] [Thread: " + threadInfo + "] " + message);

        while (menuChangeLog.size() > MAX_LOG_SIZE) {
            menuChangeLog.remove(menuChangeLog.size() - 1);
        }
    }
}